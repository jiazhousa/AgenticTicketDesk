...
| `apps/server/src/app.ts` | 装配：dispatcher 构造后 `service.onInflightReleased = (id) => dispatcher.releaseAndRecheck(id)`（**箭头包裹防 this 丢失**，setter 顺序修正）；tick 挂载=buildApp 构建 Dispatcher 时启动，**清理挂 Fastify `app.addHook('onClose')` 清 interval**（AppRuntime 无生命周期概念——r3-l1 修正） |
...
| `apps/server/test/helpers.ts` | 工厂扩 DispatcherOptions 透传（tickIntervalMs/now）**与 config 三键透传（maxConcurrentPerRepo/maxRetries/retryBackoffSec）**（r3-l5）+双并行 fake worker 支持（两单独立实例与断言钩子） |
