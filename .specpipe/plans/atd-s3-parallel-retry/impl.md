...
- **基线**：main@8456548
- **修订记录**：r1 REJECT 6 → v2（挂载/绑定/作用域/通道）；r2 REJECT 47 → **v3**（本版）：critical=卡点 BLOCKED settle 路径补挂载+测试四路径对齐 spec/回调契约写死（枚举修正+setter 注入+事务边界）/排队校验顺序与 tick 恢复语义写死/seam 注入通路贯通/块 2 补 TransitionActions+plannedFiles 录入/low 全收
...
6. **RETRY_WAIT 引擎**：报告缺失/schema 错 → IN_PROGRESS→BLOCKED('agent') + retry_count 递增；**升级判定时点写死：递增后当次立即判 `retry_count >= maxRetries`，达限则当次同态升级**（不等奖下一 tick）——pending_label 'agent'→'l3' + blockReason 历次摘要 + system comment（无 transitions 行）；未达限则 retry_at=`now()+min(retryBackoffSec×2^(retry_count-1), 240)`。5s tick 扫到期单 → **恢复顺序写死：先查闸门（自身一直在占用集，文件集不复验——BLOCKED 期间占用保留，占用集单调无新增冲突面）→ 闸门满则 retry_at 顺延一个 tickInterval 下轮再试（不落 queued 字段不转移状态）→ 有位则 BLOCKED→DISPATCHED（actor=system）重 spawn round+1**；spawn 失败走既有 preSpawnFail。continue/reassign/reopen 清零 retry_count
7. **实测防线**：settle DONE 提取基线 diff 文件清单（复用 `report.ts` 既有 git 调用通道，与 commit 提取同源）落 `ticket_files`，与占用集声明单交叉比对 → 相交两单各落 system 留言
8. **seam 注入通路（r2-m3 贯通）**：`DispatcherOptions`（现有构造 opts）扩 `tickIntervalMs?: number`（缺省 5000）与 `now?: () => number`（缺省 Date.now）；通路：测试 → `test/helpers.ts`（工厂扩 opts 透传）→ `app.ts` AppRuntime 构造 → Dispatcher；退避断言经注入 now 构造
...
| `apps/server/src/app.ts` | 装配：dispatcher 构造后 `service.onInflightReleased = dispatcher.releaseAndRecheck`（setter，顺序修正）；tick 挂载/清除进 AppRuntime 生命周期 |
...
| `apps/server/test/helpers.ts` | 工厂扩 DispatcherOptions 透传（tickIntervalMs/now）+双并行 fake worker 支持（两单独立实例与断言钩子） |
...
| `apps/web/src/components/SpecCard.tsx` | **plannedFiles 录入入口（r2-m4）**：spec 提交表单增文本域（每行一个路径，尾斜杠目录语义提示），随 submitSpec 提交；SPEC_READY 后只读展示 |
| `apps/web/src/components/TransitionActions.tsx` | 放行按钮渲染点：排队单（queuedReason 非空）禁用放行+显示 QueuedTag（详情页与工作台共用组件——r2-m4 补） |
| `apps/web/src/pages/WorkbenchPage.tsx` | 待处理区：排队单 QueuedTag+隐藏放行按钮；阻塞区按 pendingLabel 分流——l3 走 BlockedResolutionCard；agent 只读卡（重试次数/useNow 倒计时） |
...
- D3 tick=dispatcher 内 setInterval（opts：tickIntervalMs/now），AppRuntime 挂载/清除
...
- D6 退避：retry_count 进入递增（首次=1），`min(retryBackoffSec×2^(retry_count-1), 240)`；**递增后当次判升级**；continue/reassign/reopen 清零
