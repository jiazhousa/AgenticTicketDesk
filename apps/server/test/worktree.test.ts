...
describe('worktree 管理器：allocate / 复用 / reclaim【B8】', () => {
  test('allocate：建 worktree+分支，基线=默认分支 HEAD', () => {
    const { worktree, config } = createRealContext();
    const head = git(config.repoPath, ['rev-parse', 'main']);
    const p = worktree.allocate(101);
    expect(existsSync(p)).toBe(true);
    expect(p).toContain('-t101');
    expect(git(config.repoPath, ['rev-parse', '--verify', 'refs/heads/atd/t101'])).toBe(head);
    expect(worktree.baseline(p)).toBe(head);
  });

  test('allocate 复用：同单二次分配同路径，既有提交保留', () => {
    const { worktree } = createRealContext();
    const p1 = worktree.allocate(102);
    const sha = commitIn(p1, 'r1.txt');
    const p2 = worktree.allocate(102);
    expect(p2).toBe(p1);
  ...
  test('reclaim keepBranch=true：目录删/分支留/commit 可达，再分配分支延续', () => {
    const { worktree, config } = createRealContext();
    const p = worktree.allocate(103);
    const sha = commitIn(p, 'keep.txt');
    worktree.reclaim(103, true);
    expect(existsSync(p)).toBe(false);
    expect(git(config.repoPath, ['rev-parse', '--verify', 'refs/heads/atd/t103'])).toBe(sha);
    // 再分配：从既有分支挂回，提交仍在
    const p2 = worktree.allocate(103);
  ...
  test('reclaim keepBranch=false：worktree 与分支双删', () => {
    const { worktree, config } = createRealContext();
    const p = worktree.allocate(104);
    commitIn(p, 'drop.txt');
    worktree.reclaim(104, false);
    expect(existsSync(p)).toBe(false);
    expect(() => git(config.repoPath, ['rev-parse', '--verify', 'refs/heads/atd/t104'])).toThrow();
  });
});
...
describe('worktree 回收路由', () => {
  ...
  test('非终态（IN_PROGRESS）→ 422 WORKTREE_ACTIVE', async () => {
    ...
    expect(existsSync(ctx.worktree.pathFor(id))).toBe(true);
  });
  ...
  test('终态（DONE）默认 keepBranch=true：回收 200，分支保留', async () => {
    ...
    expect(existsSync(ctx.worktree.pathFor(id))).toBe(false);
    expect(() =>
      execFileSync('git', ['-C', ctx.config.repoPath, 'rev-parse', '--verify', `refs/heads/atd/t${id}`]),
    ).not.toThrow();
  });
