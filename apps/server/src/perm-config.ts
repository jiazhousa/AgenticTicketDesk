 ...
 * last-match-wins）。仅约束经 opencode 工具层的 bash/git 操作；profile 直跑外部命令不经该层，
...
export function buildPermConfig(): { permission: { bash: Record<string, string> } } {
  return { permission: { bash: { ...PERM_BASH_RULES } } };
}
